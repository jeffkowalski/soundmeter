...
class SOS_Delay_State {
  ...
  public:
    SOS_Delay_State() : w0 (0), w1 (0) {}
};

extern "C" {
int sos_filter_f32 (float const * input, float * output, int len, const SOS_Coefficients & coeffs, SOS_Delay_State & w);
}
__asm__(
    //
...
extern "C" {
float sos_filter_sum_sqr_f32 (float const *            input,
                              float *                  output,
                              int                      len,
                              SOS_Coefficients const & coeffs,
                              SOS_Delay_State &        w,
                              float                    gain);
}
__asm__(
...
class SOS_IIR_Filter {
  ...
  public:
    // Dynamic constructor
    SOS_IIR_Filter (size_t num_sos, const float _gain, const SOS_Coefficients _sos[])
        : num_sos (num_sos), gain (_gain), sos (NULL), w (NULL) {
        if (num_sos > 0) {
    ...
    template <size_t Array_Size> SOS_IIR_Filter (const float gain, const SOS_Coefficients (&sos)[Array_Size])
     ...
     */
    inline float filter (float const * input, float * output, size_t len) const {
        if ((num_sos < 1) || (sos == NULL) || (w == NULL))
            return 0;
        float const * source = input;

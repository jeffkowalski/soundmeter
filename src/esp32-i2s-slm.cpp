 ...
 *
 * Modified by Jeff Kowalski to gather statistics over an interval
...
// B = [0.477326418836803, -0.486486982406126, -0.336455844522277, 0.234624646917202,
...
// B = [-0.45733702338341309   1.12228667105574775  -0.77818278904413563, 0.00968926337978037,
...
SOS_IIR_Filter IM69D130 = {
    ...
    sos : {{-1.0, 0.0, +0.9992, 0},  // DC blocker, a1 = -0.9992
...
// B = [0.169994948147430, 0.280415310498794, -1.120574766348363, 0.131562559965936,
// 0.974153561246036, -0.282740857326553, -0.152810756202003] A = [1.0, -2.12979364760736134,
...
// B = [-0.49164716933714026, 0.14844753846498662, 0.74117815661529129, -0.03281878334039314,
// -0.29709276192593875, -0.06442545322197900, -0.00364152725482682] A = [1.0, -1.0325358998928318,
...
SOS_IIR_Filter C_weighting = {
    ...
    sos : {{+1.4604385758204708, +0.5275070373815286, +1.9946144559930252, -0.9946217070140883},
...
void loop() {
    ...
    if (xQueueReceive (samples_queue, &q, portMAX_DELAY)) {
        ...
        if (Leq_samples >= SAMPLE_RATE * LEQ_PERIOD) {
            ...
            static double Lmin  = INFINITY;
            static double Lmax  = 0;
            ...
            if (++count >= LEQ_PER_RECORD) {
                record_to_database (Lmin, Lsum / count, Lmax);
            ...
            // The 'short' Leq line
            double short_Leq_dB =
                MIC_OFFSET_DB + MIC_REF_DB + 20 * log10 (sqrt (double (q.sum_sqr_weighted) / SAMPLES_SHORT) / MIC_REF_AMPL);
            uint16_t len = min (max (0, int (((short_Leq_dB - MIC_NOISE_DB) / MIC_OVERLOAD_DB) * (display.getWidth() - 1))),
                                display.getWidth() - 1);
